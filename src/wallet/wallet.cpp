...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            // Will try to rescan it if zDOGEC upgrade is active.
            doZDogeCRescan(pindex, block, setAddedToWallet, consensus, fCheckZDOGEC);
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != 5000 * COIN) {
        strError = "Invalid collateral tx value, must be 5,000 DOGEC";
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    CzDOGECWallet* zwalletInstance = new CzDOGECWallet(walletInstance);
    ...
    if (!zwalletInstance->GetMasterSeed().IsNull()) {
        ...
        walletInstance->zdogecTracker->Init();
