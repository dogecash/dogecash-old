...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, nullptr, false, ONLY_10000, false, 1, false, false);
    ...
    }
    {
        ...
        BOOST_FOREACH (const COutputEntry& r, listReceived) {
            if (pwallet->mapAddressBook.count(r.destination)) {
                map<CTxDestination, AddressBook::CAddressBookData>::const_iterator mi = pwallet->mapAddressBook.find(r.destination);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, bool fIncludeColdStaking, bool fIncludeDelegated) const
{
    ...
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, ALL_COINS, useIX, 1, fIncludeColdStaking, fIncludeDelegated);
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        //if(out.tx->vout[out.i].nValue == Params().MasternodeCollateralLimit()*COIN) continue;
...
set<CTxDestination> CWallet::GetAccountAddresses(string strAccount) const
{
    ...
    BOOST_FOREACH (const PAIRTYPE(CTxDestination, AddressBook::CAddressBookData) & item, mapAddressBook) {
...
bool CWallet::CreateZerocoinSpendTransaction(
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (changeAddress) {
                scriptZerocoinSpend = GetScriptForDestination(changeAddress->Get());
                if (nChange) {
                    // Reserve a new key pair from key pool
                    CPubKey vchPubKey;
