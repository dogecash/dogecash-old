...
int64_t GetBlockValue(int nHeight)
{
	...
	nSubsidy = 0 * COIN;
        ...
        nSubsidy = 10.8 * COIN;
	} else if (nHeight <= 768221 && nHeight >= 238622) {
        nSubsidy = 9 * COIN;
	} else if (nHeight <= 1289222 && nHeight >= 768222) {
        nSubsidy = 5.4 * COIN;
	}    else {
        nSubsidy = 5.4 * COIN;
	}
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 DOGEC.
...
int ActiveProtocol()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) || sporkManager.IsSporkActive(SPORK_18_NEW_PROTOCOL_ENFORCEMENT_3))
