...
CzDOGECTracker::CzDOGECTracker(CWallet* parent)
{
    this->wallet = parent;
...
{
    bool iszDOGECWalletInitialized = (nullptr != zDOGECWallet);
    CMintMeta meta;
    ...
    if (!iszDOGECWalletInitialized)
        zDOGECWallet = new CzDOGECWallet(wallet);
...
std::set<CMintMeta> CzDOGECTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzdogec from DB\n", __func__, listDeterministicDB.size());
    }
