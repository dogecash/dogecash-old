...
extern UniValue DoZdogecSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzdogecseed(const JSONRPCRequest& request);
extern UniValue getzdogecseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzdogec(const JSONRPCRequest& request);
extern UniValue dzdogecstate(const JSONRPCRequest& request);
