...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zdogecObj.pushKV(std::to_string(denom), ValueFromAmount(0));
        else
            zdogecObj.pushKV(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN)));
    }
    zdogecObj.pushKV("total", ValueFromAmount(GetZerocoinSupply()));
    obj.pushKV("zDOGECsupply", zdogecObj);
