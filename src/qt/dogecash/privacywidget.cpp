...
#include "qt/dogecash/forms/ui_privacywidget.h"
#include "qt/dogecash/qtutils.h"
#include "guiutil.h"
#include "qt/dogecash/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(DogeCashGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zDOGEC anonymizes your DOGEC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zDOGEC or convert back to DOGEC"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zDOGEC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zDOGEC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zDOGEC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zDOGEC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zDOGEC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zDOGEC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zDOGEC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zDOGEC");
    ...
    // Only Convert to DOGEC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzDOGEC->setTitleClassAndText("btn-title-grey", "Total 0 zDOGEC");
    ui->btnTotalzDOGEC->setSubTitleClassAndText("text-subtitle", "Show denominations of zDOGEC owned.");
    ui->btnTotalzDOGEC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select DOGEC outputs to mint into zDOGEC.");
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zDOGEC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DOGEC to mint into zDOGEC"));
    }else{
        btnText = tr("Convert back to DOGEC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zDOGEC to convert back into DOGEC"));
    }
...
void PrivacyWidget::onTotalZdogecClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzDOGEC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzDOGEC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zDOGEC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZdogec(
        ...
        // Spend succeed
        inform(tr("zDOGEC converted back to DOGEC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
    ...
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            if (!coinControlDialog) {
                ...
                coinControlDialog->updateCoinControlLabels();
            }
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzDOGEC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
