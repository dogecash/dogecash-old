    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (DOGEC) or private coins (zDOGEC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a DogeCash address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 DOGEC");
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend PIV (not zdogec)
...
bool SendWidget::sendZdogec(QList<SendCoinsRecipient> recipients){
    ...
    if (walletModel->sendZdogec(
        ...
        emit message("zDOGEC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
