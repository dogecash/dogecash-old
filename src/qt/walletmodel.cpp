...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DOGEC.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZdogecSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZdogec(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzdogecReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzdogecReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzdogecReset.connect(boost::bind(NotifyzdogecReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzdogecReset.disconnect(boost::bind(NotifyzdogecReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid DOGEC address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid DOGEC address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from DOGEC address");
