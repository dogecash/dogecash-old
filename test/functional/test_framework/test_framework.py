...
class DogeCashTestFramework():
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 DOGEC) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 DOGEC), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 DOGEC), change = 6750 - 6666 - fee
