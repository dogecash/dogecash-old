...
class ZerocoinSpendTest(DogeCashTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zdogec_bal, dogec_bal):
            zdogec_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zdogec_bal)
            dogec_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], dogec_bal)
            return zdogec_bal, dogec_bal
