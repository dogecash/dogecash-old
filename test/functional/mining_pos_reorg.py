...
class ReorgStakeTest(DogeCashTestFramework):
    ...
    def check_money_supply(self, expected_dogec, expected_zdogec):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected DOGEC and zDOGEC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_dogec))
            for denom in node['zDOGECsupply']:
                assert_equal(node['zDOGECsupply'][denom], DecimalAmt(expected_zdogec[denom]))
    ...
    def run_test(self):
        ...
        # Check DOGEC and zDOGEC supply at the beginning
        # ------------------------------------------
        # zDOGEC supply: 2 coins for each denomination
        expected_zdogec_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # DOGEC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zdogec_supply)
        ...
        # Verify that DOGEC and zDOGEC supplies were properly updated after the spends and reorgs
        self.log.info("Check DOGEC and zDOGEC supply...")
        ...
        expected_zdogec_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zdogec_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zdogec_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zdogec_supply)
